...
namespace PhamaceySystem.Forms.Person_Forms
{
    public partial class F_Donars : F_Master_List
    {
        ...
        public override void Insert_Data()
        {
            try
            {
                if (Validate_Data())
                {
                    ...
                    C_Add_System_record.Add(tit, "إضافة", $" تم إضافة {tit}  باسم {TF_Pers_Donars.Donar_name} ");
            ...
            try
            {
                if (Is_Double_Click)
                {
                    if (Validate_Data() && gv.RowCount > 0 && TF_Pers_Donars != null)
                    {
                        ...
                        C_Add_System_record.Add(tit, "تعديل", $" تم تعديل {tit}  باسم {TF_Pers_Donars.Donar_name} ");
                ...
                if (Is_Double_Click)
                {
                    if (C_Master.Qustion_Massege_Box(C_Master.mas_del) == DialogResult.Yes)
                    {
                        if (gv.RowCount > 0)
                            ...
                            {
                                Get_Row_ID(row_id);
                                cmdDonars.Delete_Data(TF_Pers_Donars);
                                C_Add_System_record.Add(tit, "حذف", $" تم حذف {tit}  باسم {TF_Pers_Donars.Donar_name} ");
        ...
        private void Fill_Graid()
        {
            ...
            gv.Columns["name"].Caption = "اسم المتبرع";
            gv.BestFitColumns();
